...
/**
 * Converts a given value to a string and gets a character from that string
 */
...
/**
 * Creates the state used by obfuscateString
 */
function _createEncoderState (bindLookupsTo = "_") {
  return {
    sequences: [
      { result: "true",      logic: "!![]" },
      { result: "false",     logic: "![]" },
...
/**
 * Obfuscates a commonly-available character if possible
 */
...
/**
 * Obfuscates a string made of commonly-available characters
 */
...
/**
 * Sets up a lookup mechanism for arbitrary strings using advanced type
 * coersion and obscure JS APIs
 */
 ...
 */
function obfuscateString (str, bind) {
  const state = _createEncoderState(bind);
